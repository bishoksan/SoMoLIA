        ...
        try {
            if (bExpr.isQuantifier()) {
                ...
                for (int i = 0; i < varNames.length; i++) {
                    ...
                    bvVarExpr[nrBoundVars - 1 - i] = ctx.mkBVConst(varNames[i], Main.getINT_WIDTH()); //change it with appropriate bit
                }
...
//    }
    ...
    public ArithExpr convertToLIAExpr(BitVecExpr bvExpr, Context ctx) {
        ...
        try {
            ...
            if (bvExpr.isApp() && bvExpr.getNumArgs() == 0 && bvExpr.isBV()) //is bv variable
            {
            ...
            if (bvExpr.isBVMul()) {
                ...
                for (Expr e : exprs) {
                    arithAcc = ctx.mkMul(arithAcc, convertToLIAExpr((BitVecExpr) e, ctx));
                }
                return arithAcc;
            }
            ...
            //return convertToLIAExpr(bvExpr,  ctx) ;
             if(bvExpr.isBVToInt()){
