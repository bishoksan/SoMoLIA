...
//            return null; //all delta coeff are tried, and find a different disjunct, 
        ...
        for (String id : currentAssumptions) {
            ...
            if (!isTrueFormula) { //do not add true formula
...
//    BoolExpr[] getCurrentAssumptionUnder(Model m, Context context) {
//        BoolExpr assumptionFormula = context.mkTrue();
//        ArrayList<BoolExpr> assumptions = new ArrayList<>();
...
//                assumptions.add(boolId);
//                assumptionFormula = context.mkAnd(assumptionFormula, boolId);
//            };
//        }
//        cutAssumptions = (BoolExpr) assumptionFormula.simplify();
//        return assumptions.toArray(new BoolExpr[assumptions.size()]);
//    }
    //highly inefficient
...
//        System.out.println(inputFile);
        try {
            logger = Logger.getLogger(SATBendersDecomp.class);
            ...
            //get smt-logic
            BoolExpr bExpr = z3Interface.readSMTLIB2File(inputFile, context);

            //process bit-vector formulas
            if (Main.SMT_LOGIC.equals("QF_BV") || Main.SMT_LOGIC.equals("QF_AUFBV")) {
...
//                return ctx.mkImplies(idBe, (BoolExpr) ctx.mkFalse()); //avoiding this to appear in the boolean model
    ...
    /**
     * the exprs has the form a*x + ...+b*y, if the coeffs(a..b>some threshold)
     * then we produce (M-a)*x+...+(M-b)*y
     */
//    public ArithExpr reduceCoeffExpr(ArithExpr ae){
//        ae=(ArithExpr)ae.simplify();
//        Expr[] exprs;
//        
//        if(ae.isMul()){//following z3, the first is a constant and the second is a variable
//            exprs=ae.getArgs();
//            
//        }
//        if(ae.isAdd()){
